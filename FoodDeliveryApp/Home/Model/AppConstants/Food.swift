...
struct Food {
    static func getArray(type: FoodType, index: Int? = nil) -> [FoodCard] {
        var foodArray = [FoodCard]()
        switch type {
        case .pizza:
        ...
        case .cake:
            return getCakeArray()
        case .iceCream:
            foodArray = getIceCreamArray()
        }
        if let index = index, index < foodArray.count {
    ...
    private static func getBurgerArray() -> [FoodCard] {
        return [
